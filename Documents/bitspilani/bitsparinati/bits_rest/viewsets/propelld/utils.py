...
def get_propelld_inprogress(email):
	try:
		...
		ea = PropelldApplication.objects.filter(application__login_email__email=email).latest('created_on')
		if ea.status in ['DROPPED','REJECTED', 'New Loan Application',]:
			ea = None
...
def get_PropelldApplication_declined(email):
	try:
		ea = PropelldApplication.objects.filter(status__in=['FAILED'], application__login_email__email=email)
	except PropelldApplication.DoesNotExist as e:
		ea = None
